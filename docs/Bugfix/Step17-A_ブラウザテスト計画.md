...
前提コミット: `43ed673 refactor: Step 17-A-10 の owner 境界整理とコメント同期を反映する` / `75076ff docs: Step 17-A-10 完了とStep 17-Bへの引き継ぎを記録する` / `490c9c4 refactor: extensionEnabled を永続設定から runtime state へ分離する`
...
加えて、`extensionEnabled` は sync storage の永続設定ではなく、現在の playback session における `state.extensionEnabled` を正本とする runtime state へ分離した。  
そのため今回のブラウザテストでは、**見た目の確認だけでなく、開閉・破棄・復帰・再初期化・遷移の各経路が壊れていないこと**を確認する。

特に次の観点を満たすかを確認する。

- 動画再生中に native toggle から拡張機能を runtime ON/OFF でき、状態が current playback session 内で一貫すること
...
- `extensionEnabled` が sync storage へ保存されず、新しい playback session では runtime state として初期化されること
...
- sync storage に `extensionEnabled` が保存値として残っていないこと

### 3-3. 主な観測対象

| 項目 | 役割 | 主な確認ポイント |
| :-- | :-- | :-- |
| `atvb-native-toggle` | 拡張全体の runtime ON/OFF | `state.extensionEnabled` と表示が一致し、OFF 時も残って再操作できるか |
...
| `state.extensionEnabled` | 拡張全体の runtime state 正本 | native toggle 後に更新され、sync storage には保存されないか |
...
| 4 | 拡張 OFF 確認 | runtime state 反映、dispose / cleanup、非永続化の確認 | 高 |
| 5 | OFF → ON 復帰確認 | runtime state 反映、再生成と復帰の確認 | 高 |
| 6 | seek 後の追従確認 | block 再同期と panel / overlay 更新確認 | 高 |
| 7 | 再初期化系操作確認 | reset / reinitialize 契約の確認 | 中 |
| 8 | SPA 遷移確認 | cleanup、新セッション初期化、runtime state 非復元の確認 | 高 |
...
4. DevTools で `state.extensionEnabled` が `false` として扱われていることを確認する
5. sync storage に `extensionEnabled` が保存・更新されていないことを確認する
...
- native toggle 操作後、`state.extensionEnabled` が `false` として反映され、OFF 用 cleanup が一度だけ実行される
- `extensionEnabled` が sync storage に保存・更新されない
- Console に cleanup エラーが出ない

**異常時に疑う箇所**

- `settings-runtime.js` の `SETTINGS_CHANGED` 処理
- `panelUi.dispose()`
- `modules/panel-ui.js`
...
4. ON/OFF を数回連続で切り替える

**期待結果**

- ON→OFF→ON の基本復帰経路が成立する
- panel / overlay / subtitle が正常に再生成される
- native toggle 操作後、`state.extensionEnabled` が `true` として反映され、再初期化・UI 再生成が一度だけ実行される
- ON/OFF を連続操作しても、送信中の二重 toggle、二重 host、二重 listener、二重描画が発生しない
- Console に restart / cleanup 関連のエラーが出ない
...
5. 遷移前に拡張を OFF にした場合も同じ確認を行う

**期待結果**

- 遷移前の panel host / overlay host / observer / listener が残らない
- 遷移先で新しい session として正常に初期化される
- SPA 遷移時の cleanup 多重実行が起きない
- 遷移先では前セッションの `extensionEnabled` 状態を sync storage から復元しない
- 新しい playback session は、その初期化時点の `state.extensionEnabled` に従って UI を構築する
- Console に旧セッションを参照するエラーが出ない

**異常時に疑う箇所**

- `runtime-observers.js`
- `reinitialize-coordinator.js`
- `settings-runtime.js` の `loadSettingsFromSync()`
...
- ON/OFF 後の panel / overlay / native toggle の表示状態が `state.extensionEnabled` と矛盾しない

**異常時に疑う箇所**

- `modules/panel-ui.js` の listener / observer / timer cleanup
- `modules/playback-session-cleanup.js`
- `settings-runtime.js`
- `content.js`
- `runtime-observers.js`
...
- 実施日時
- 対象作品・シーズン・エピソード
- primary / secondary 言語設定
- panel 初期状態
- 操作順
- 結果（OK / NG）
- Console エラーまたは警告
- DOM 上で確認した host / toggle の状態
- 再現性
- スクリーンショットまたはログ保存先

runtime ON/OFF を含むシナリオでは、次も記録する。

- toggle 操作直前・直後の `state.extensionEnabled` の扱い
- `extensionEnabled` が sync storage に残っていないこと
- `APPLY_SETTINGS_TO_APPLE_TV` 後に cleanup または再起動が一度だけ走ったこと
- panel host / overlay host の個数と残骸の有無

***

## 8. 異常時の初動

不具合を確認した場合は、次の順で観測情報を残す。

1. 操作手順と発生タイミングを記録する
2. Console error / warning を保存する
3. panel host / overlay host / native toggle の DOM 状態を確認する
4. `state.extensionEnabled` と native toggle 表示の一致を確認する
5. 同じ作品・同じ操作順で再現するかを確認する
6. seek、panel 開閉、ON/OFF、SPA 遷移のどれが起点かを分ける
7. 関連する debug log を保存する
