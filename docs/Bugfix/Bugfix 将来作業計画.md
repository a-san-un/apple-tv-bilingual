...
**更新方針:** 完了済み事項や今回の整理対象は記載しない。 必要になった時点で、この文書から新しい実装シートへ移す。 今回は、`settings-runtime.js` の起動責務移管、`content.js` の主要 direct cleanup 解消、`panel-ui.js` / `debug-panel-runtime.js` の説明層整理は完了済みとして将来計画から外し、**cleanup owner 一本化の残差整理が終わった後に着手する項目だけ**を残す。
...
- 途中で不要になった検討メモ。
- 現在の主作業に属するチェックリスト。
- `settings-runtime.js` の direct start 経路整理、`coordinator.attachAndMaybeStart(...)` への起動一本化、`content.js` の主要 direct cleanup callsite 集約のように、今回すでに反映済みの責務移管項目。
- `modules/panel-ui.js` / `modules/debug-panel-runtime.js` の説明層更新のように、session subordinate module 化が完了した項目。
...
### A. cleanup owner 一本化完了後の構造整理

今回の修正で cleanup owner 一本化まで完了した後に、モジュール構成全体をさらに整理するフェーズである。
...
- cleanup owner 完了後を前提にした `content.js` の wiring / request 境界の再確認。

着手条件:

- `reinitialize-coordinator.js` に残る `clearInternalSubtitleState(...)` 直呼び除去が完了していること。
- `content.js` に残る helper / DI の縮退が完了していること。
- 実機検証で重大な regress が残っていないこと。

***

### B. lifecycle・cleanup の長期検証

今回の cleanup 修正が一通り完了したあと、長時間再生や複数遷移を含む条件で安定性を確認するフェーズである。
...
- `modules/playback-session-cleanup.js` が実装・説明の両方で唯一入口として読めること。
...
主作業の修正が固まったあとで、ログの責務と粒度を再設計するフェーズである。
...
- `startup = coordinator`、`cleanup = playback-session-cleanup` の境界がコードと文書の両方で安定していること。
...
現時点で「今はまだ将来計画ではなく、進行中の主作業に属する」と判断する項目は次のとおりである。

- `reinitialize-coordinator.js` に残る `clearInternalSubtitleState(...)` 直呼び除去。
- `content.js` に残る `clearInternalSubtitleState(...)` helper / DI の縮退。
- `modules/playback-session-cleanup.js` を実装・説明の両方で唯一入口として揃える最終確認。
- 不要になった watcher / timer / wrapper / 補助 cleanup の物理削除。

これらは将来計画へ送らず、**現在の cleanup owner 一本化フェーズの完了条件**として実装系ドキュメントで追い続ける。
...
|---|---|---|---|
| A | cleanup owner 一本化完了後の構造整理 | 保留 | `reinitialize-coordinator.js` / `content.js` の残差整理完了  |
...
今回の更新でも、起動入口一本化や panel/debug module の説明層整理のように完了済みの責務移管項目は、この文書には残さない方針を維持する。
