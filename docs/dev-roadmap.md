...
1. #24 / Issue #32 Phase J: secondary recovery の復帰ラグ調整と observer 整理の継続
...
この段階では「secondary が戻らないケースを壊さず扱える」基盤は入ったが、miss limit 値や primary-only fallback 条件の微調整は後続の調整対象とする。
