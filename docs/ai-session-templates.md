...
- Perplexity（このチャット）の役割:
  - 対象リポジトリ / ブランチ / docs を読み、**現在位置の整理**を担当する。
  - issue / Phase / roadmap / 分割方針をもとに、**タスクの切り分け・方針整理**を行う。
  - VS Code 側の Copilot / Serena に渡すための、**調査 / 実装 / docs 更新用の指示文テンプレ**を組み立てる。
  - 「変更しない範囲」「完了条件」「不要コード削除・重複回避」などの**安全策を明文化**する。

- VS Code の GitHub Copilot / Serena の役割:
  - このチャットから渡された指示文に従って、**コード調査 / 実装 / docs 反映 /コミット準備**を担当する。
